<?php
  ...
  $team=$_POST['team'];
      ...
      if($allow1!=0 && $allow2!=0 && $allow3!=0)
      {
        ...
        $sql = $connect->prepare("INSERT INTO table1(team,name1,dept1,year1,roll1,
        name2,dept2,year2,roll2,name3,dept3,year3,roll3)
        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");

        $sql->bind_param("sssssssssssss",$team,$name[0],$dept[0],$year[0],$roll[0],$name[1],$dept[1],$year[1],$roll[1],$name[2],$dept[2],$year[2],$roll[2]);
        $sql->execute();
